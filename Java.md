...
> 2. Runnable比较多用，因为:  
>- 1) 一个类可以继承多个接口;   
>- 2) 可以实现资源共享。eg：定义了两个线程，资源是共享的，共卖出6张票。 
...
2. wait(long timeout)
> 让当前线程处于等待(阻塞)状态，并释放同步锁，直到其他线程调用此对象的notify()或 notifyAll()，或者超过指定的时间量”，方可被唤醒(进入“就绪状态”)
...
### 八. java8新特性
1. Lambda表达式: 即在Java中引入函数式编程。   

2. 接口中可以定义默认(default)和静态方法,并能提供实现的方法体。
